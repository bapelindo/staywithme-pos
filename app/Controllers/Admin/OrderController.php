...
class OrderController extends Controller {
    ...
    public function index() {
        ...
        if ($statusFilter === 'all' || empty($statusFilter)) {
        ...
        } elseif (in_array($statusFilter, $allowedStatusesForFilter)) {
            ...
            $orders = $orderModel->getAllOrdersPaginated($limit, $offset);
            $totalOrders = $orderModel->countAllOrders();
             SessionHelper::setFlash('error', 'Filter status tidak valid.');
        }
    ...
    public function getNewOrders() {
        ...
        $orderModel = new Order();
        $newOrders = $orderModel->getNewOrdersSince($lastSeenId, 'pending_payment');

        $ordersData = array_map(fn($order) => [
            'id' => $order['id'],
            'order_number' => $order['order_number'],
            'table_number' => $order['table_number'] ?? 'N/A',
            'order_time' => DateHelper::timeAgo($order['order_time'])
        ], $newOrders);
    ...
    public function invoice(int $order_id) {
     ...
     /**
      * Memproses pembayaran cash (dipanggil dari tombol "Konfirmasi Bayar (Cash)").
      * Method ini menjadi titik awal integrasi dengan sistem kasir.
      */
     public function processCashPayment(int $order_id) {
         ...
         // Panggil createPayment (yang sudah diintegrasikan dengan sistem kasir)
         if ($paymentModel->createPayment($safeOrderId, $amountPaid, 'cash', $userId)) {
             SessionHelper::setFlash('success', 'Pembayaran tunai berhasil dikonfirmasi. Pesanan diteruskan ke dapur.');
         } else {
             SessionHelper::setFlash('error', 'Gagal memproses pembayaran. Pastikan sesi kasir Anda aktif.');
         }
