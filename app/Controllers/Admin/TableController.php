...
class TableController extends Controller {
    ...
    public function create() { $this->view('admin.tables.create', ['pageTitle' => 'Tambah Meja Baru'], 'admin_layout'); }
    ...
    public function store() {
         ...
         if ($tableId) { SessionHelper::setFlash('success', 'Meja ditambahkan.'); UrlHelper::redirect('/admin/tables'); }
         else { SessionHelper::setFlashData('old_input', $_POST); SessionHelper::setFlash('error', 'Gagal tambah meja (nomor duplikat?).'); UrlHelper::redirect('/admin/tables/create'); }
    }
    ...
    public function edit(int $id) {
         ...
         $this->view('admin.tables.edit', ['pageTitle' => 'Edit Meja: ' . SanitizeHelper::html($table['table_number']), 'table' => $table ], 'admin_layout');
    }
     ...
     public function update(int $id) {
         ...
         $tableNumber = SanitizeHelper::string($_POST['table_number'] ?? ''); $description = SanitizeHelper::string($_POST['description'] ?? null); $isActive = isset($_POST['is_active']) ? true : false;
         if (empty($tableNumber)) {
             SessionHelper::setFlashData('old_input', $_POST + ['id' => $safeId]); // PERBAIKAN: Gunakan setFlashData
             SessionHelper::setFlash('error', 'Nomor Meja wajib diisi.');
         ...
         if ($success) { SessionHelper::setFlash('success', 'Meja diperbarui.'); UrlHelper::redirect('/admin/tables'); }
         else { SessionHelper::setFlashData('old_input', $_POST + ['id' => $safeId]); SessionHelper::setFlash('error', 'Gagal update meja (nomor duplikat?).'); UrlHelper::redirect('/admin/tables/edit/' . $safeId); }
     }

    public function destroy(int $id) {
        $safeId = SanitizeHelper::integer($id); if ($safeId <= 0 || $_SERVER['REQUEST_METHOD'] !== 'POST') { UrlHelper::redirect('/admin/tables'); return; }
        $table = $this->tableModel->findById($safeId); $tableName = $table ? $table['table_number'] : 'ID ' . $safeId;
        if ($this->tableModel->deleteTable($safeId)) { SessionHelper::setFlash('success', 'Meja "' . SanitizeHelper::html($tableName) . '" dihapus.'); }
        else { SessionHelper::setFlash('error', 'Gagal hapus meja (mungkin terkait pesanan?).'); }
