...
class SessionHelper {
    ...
    // --- Menampilkan Pesan Flash (String) dengan Auto-Hide JS ---
     public static function displayFlash(
         string $key,
         string $cssClassBase = 'fixed top-5 right-5 z-50 max-w-sm w-full p-4 rounded-md border shadow-lg text-sm', // <-- Kelas untuk overlap
         array $cssClassMap = []
     ): void {
     ...
     public static function displayAllFlashMessages(
          ...
          $defaultMap = ['success' => '', 'error' => '', 'warning' => '', 'info' => '']; $keysToCheck = !empty($cssClassMap) ? array_keys($cssClassMap) : array_keys($defaultMap);
          foreach ($keysToCheck as $key) { self::displayFlash($key, $cssClassBase, $cssClassMap); }
      }
}
?>