...
class Dashboard extends Model
{
    ...
    private function getDateCondition($period, $date = null)
    {
        ...
        switch ($period) {
            case 'weekly':
                return "YEARWEEK(order_time, 1) = YEARWEEK($baseDate, 1)";
            case 'monthly':
                return "YEAR(order_time) = YEAR($baseDate) AND MONTH(order_time) = MONTH($baseDate)";
            default: // daily
                return "DATE(order_time) = DATE($baseDate)";
        }
    }
    ...
    public function getMonthToDateSales($date = null)
    {
        ...
        $sql = "SELECT COALESCE(SUM(total_amount), 0) as mtd_sales
                ...
                AND DATE(order_time) >= DATE_FORMAT($baseDate, '%Y-%m-01') 
                AND DATE(order_time) <= DATE($baseDate)";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
    ...
    public function getSalesChartData($period, $date = null)
    {
        ...
        switch ($period) {
            case 'weekly':
                $groupBy = "DAYOFWEEK(order_time)";
                $labelKey = "DAYOFWEEK(order_time)";
                ...
                break;
            case 'monthly':
                $groupBy = "DAY(order_time)";
                $labelKey = "DAY(order_time)";
                ...
                break;
            default: // daily
                $groupBy = "HOUR(order_time)";
                $labelKey = "HOUR(order_time)";
                for ($i = 0; $i < 24; $i++) {
        ...
        $sql = "
            ...
            FROM (
                ...
                WHERE 
                    ...
                    order_time >= DATE_SUB(CURRENT_DATE, INTERVAL 90 DAY)
                GROUP BY DATE(order_time)
            ) as daily_sales
