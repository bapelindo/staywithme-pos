...
<div class="container mx-auto space-y-8 mt-8">
    ...
    <!-- Metrics Grid -->
    <div class="space-y-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <!-- Total Penjualan -->
        <div class="bg-white rounded-xl shadow-md p-6 transform hover:-translate-y-1 transition-transform duration-300">
            <h3 class="text-sm font-medium text-slate-500">Total Penjualan</h3>
            <p class="text-2xl font-semibold text-slate-800 mt-8"><?= NumberHelper::format_rupiah($metrics['total_sales']) ?></p>
        </div>
        <!-- Akumulasi Bulan Ini -->
        <div class="bg-white rounded-xl shadow-md p-6 transform hover:-translate-y-1 transition-transform duration-300">
            <h3 class="text-sm font-medium text-slate-500">Akumulasi Bulan Ini</h3>
            <p class="text-2xl font-semibold text-slate-800 mt-2"><?= NumberHelper::format_rupiah($mtd_sales) ?></p>
        </div>
        <!-- Proyeksi Bulan Ini -->
        <div class="bg-white rounded-xl shadow-md p-6 transform hover:-translate-y-1 transition-transform duration-300">
            <h3 class="text-sm font-medium text-slate-500">Proyeksi Bulan Ini</h3>
            <p class="text-2xl font-semibold text-slate-800 mt-2"><?= NumberHelper::format_rupiah($monthly_projection) ?></p>
        </div>
        <!-- Status Pembayaran -->
        <div class="bg-white rounded-xl shadow-md p-6 transform hover:-translate-y-1 transition-transform duration-300">
            <h3 class="text-sm font-medium text-slate-500 mb-8">Status Pembayaran</h3>
            <div class="flex items-center space-x-4">
                <div>
                    <p class="text-lg font-semibold text-green-600"><?= NumberHelper::format_rupiah($metrics['paid_sales']) ?></p>
                    <span class="text-xs text-slate-400 mt-1">Terbayar</span>
                </div>
                <div class="border-l h-6 border-slate-200"></div>
                <div>
                    <p class="text-lg font-semibold text-amber-600"><?= NumberHelper::format_rupiah($metrics['unpaid_sales']) ?></p>
                    <span class="text-xs text-slate-400 mt-1">Belum Dibayar</span>
                </div>
            </div>
        </div>
        <!-- Transaksi & Produk -->
        <div class="bg-white rounded-xl shadow-md p-6 transform hover:-translate-y-1 transition-transform duration-300 lg:col-span-2">
             <h3 class="text-sm font-medium text-slate-500 mb-2">Detail Transaksi</h3>
             <div class="flex items-center space-x-4">
                <div>
                    <p class="text-lg font-semibold text-slate-800"><?= number_format($metrics['transactions']) ?></p>
                    <span class="text-xs text-slate-400 mt-1">Total Transaksi</span>
                </div>
                 <div class="border-l h-6 border-slate-200"></div>
                <div>
                    <p class="text-lg font-semibold text-slate-800"><?= number_format($metrics['total_products']) ?></p>
        ...
        <div class="bg-white rounded-xl shadow-md p-6 transform hover:-translate-y-1 transition-transform duration-300 lg:col-span-2">
            <h3 class="text-sm font-medium text-slate-500 mb-4">Rata-rata per Transaksi</h3>
            <div class="flex items-center space-x-4">
                <div>
                    <p class="text-lg font-semibold text-slate-800"><?= NumberHelper::format_rupiah($metrics['sales_per_transaction']) ?></p>
                    <span class="text-xs text-slate-400 mt-1">Penjualan</span>
                </div>
                <div class="border-l h-6 border-slate-200"></div>
                <div>
                    <p class="text-lg font-semibold text-slate-800"><?= number_format($metrics['products_per_transaction'], 2) ?></p>
