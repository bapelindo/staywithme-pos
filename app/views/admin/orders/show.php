<?php
// File: app/Views/admin/orders/show.php (Revisi Hapus Paid)
...
$statusClasses = [
    ...
    'served' => 'bg-green-100 text-green-700', // Ini status akhir utama
    // 'paid' dihapus
...
$statusTexts = [
    ...
    'preparing' => 'Disiapkan',
    'ready' => 'Siap',
    'served' => 'Disajikan', // Ini status akhir utama
    // 'paid' dihapus
    'cancelled' => 'Batal'
];

// === PERUBAHAN: Kondisi untuk menyembunyikan tombol ===
// Sembunyikan tombol jika sudah 'served' atau 'cancelled'
...
<div class="max-w-4xl mx-auto">
    <?php // ... Bagian atas (link kembali, tombol invoice) tetap sama ... ?>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden border border-slate-200">
        <?php // Header Order Detail (Tetap Sama) ?>
        <div class="p-5 border-b border-slate-200 bg-slate-50">
             <div class="flex flex-col sm:flex-row justify-between sm:items-center">
                 ...
                 <div class="mt-3 sm:mt-0 text-center sm:text-right">
                     <span class="inline-block text-sm font-semibold px-3 py-1 rounded-full <?= $statusClasses[$order['status']] ?? 'bg-gray-100 text-gray-600' // Fallback style ?>">
                         <?= SanitizeHelper::html($statusTexts[$order['status']] ?? ucfirst($order['status'])) ?>
                     </span>
                 </div>
             </div>
             <?php // ... Catatan Pelanggan ... ?>
        </div>

        <?php // Item Dipesan (Tetap Sama) ?>
        <div class="p-5">
            <?php // ... Tabel Item ... ?>
        </div>

        <?php // Informasi Pembayaran ?>
         <div class="p-5 border-t border-slate-200">
             ...
             <?php elseif($order['status'] === 'pending_payment'): ?>
                 ...
                 <?php // Tombol "Tandai Lunas" hanya muncul jika status pending_payment ?>
                 ...
                 <?php endif; ?>
             <?php else: // Status lain (received, preparing, ready, served) - Anggap sudah bayar ?>
                 <p class="text-sm text-green-600">Pembayaran sudah dikonfirmasi.</p>
                 <?php // Tampilkan detail payment jika ada (untuk kasus dimana payment dicatat tapi status belum 'paid') ?>
                 <?php if($payment): ?>
        ...
        <?php // === PERUBAHAN: Kondisi untuk Tombol Ubah Status === ?>
        <?php if (!$isFinalOrCancelled && AuthHelper::getUserRole() !== 'kitchen'): // Sembunyikan jika served atau cancelled ?>
         <div class="p-5 border-t border-slate-200 bg-slate-50">
              ...
              <div class="flex flex-wrap gap-2">
                  ...
                  // Hapus 'paid' jika sudah dihapus dari enum
                  $possibleNextStatuses = ['received', 'preparing', 'ready', 'served', 'cancelled'];
                  ?>
                  <?php foreach($possibleNextStatuses as $nextStatus): ?>
                      <?php // Jangan tampilkan tombol untuk status saat ini atau jika status tidak valid ?>
...
<?php // ... (Kode JavaScript untuk tombol change-status-btn tetap sama) ... ?>
<script>
document.addEventListener('DOMContentLoaded', () => {
    // Event listener untuk tombol ubah status (jika ada)
    document.querySelectorAll('.change-status-btn').forEach(button => {
        button.addEventListener('click', async (e) => {
            ...
            const currentButtonText = btn.textContent; // Simpan teks asli

            if (!confirm(`Ubah status pesanan ke "${newStatus}"?`)) return;

            btn.disabled = true;
            btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>'; // Tampilkan spinner
            ...
            try {
                ...
                if (response.ok && result.success) {
                     ...
                     setTimeout(() => window.location.reload(), 1000); // Reload halaman
                 } else {
                 ...
                 btn.disabled = false;
                 btn.innerHTML = currentButtonText; // Kembalikan teks tombol asli
            }
        });
    });
});
