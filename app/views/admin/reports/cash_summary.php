...
<div class="container mx-auto p-4 lg:p-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200 mb-8">
        <form action="<?= UrlHelper::baseUrl('admin/reports/cashier/cash-summary') ?>" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200 flex items-center justify-between">
            <div>
                <h3 class="text-sm font-medium text-gray-500">Total Kas Kasir</h3>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Masuk (Rp)</th>
                        <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Keluar (Rp)</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Login</th>
                    ...
                    <?php if (empty($cashTransactions)): ?>
                        <tr>
                            <td colspan="7" class="text-center py-12 text-gray-500">
                                <i class="fas fa-money-bill-transfer text-4xl mb-3 text-gray-300"></i>
                                <p class="font-medium">Tidak ada data transaksi kas untuk ditampilkan.</p>
                                <p class="text-sm">Silakan sesuaikan filter tanggal atau pencarian Anda di atas.</p>
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($cashTransactions as $transaction): ?>
                            <tr>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"><?= SanitizeHelper::html($transaction['category'] ?? '-') ?></td>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-700"><?= SanitizeHelper::html($transaction['device_name'] ?? '-') ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
