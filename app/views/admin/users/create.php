        ...
        <form action="<?= UrlHelper::baseUrl('/admin/users/store') ?>" method="POST" novalidate>
            ...
            <div class="mb-4">
                ...
                <select id="role" name="role" required
                    ...
                    <option value="" disabled <?= empty($oldInput['role']) ? 'selected' : '' ?>>-- Pilih Peran --</option>
                    <?php foreach ($roles as $role): ?>
                        <option value="<?= SanitizeHelper::html($role) ?>"
                                <?= (isset($oldInput['role']) && $oldInput['role'] === $role) ? 'selected' : '' ?>>
                            <?= SanitizeHelper::html(ucfirst($role)) // Tampilkan dengan huruf besar di awal ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="mb-6">
                 <label for="is_active" class="flex items-center">
                     <input type="checkbox" id="is_active" name="is_active" value="1"
                            ...
                            <?= (isset($oldInput['is_active']) || empty($oldInput)) ? 'checked' : '' // Default checked ?>
                     >
                     <span class="ml-2 text-sm text-slate-700">Akun Aktif</span>
                 </label>
                 <p class="text-xs text-slate-500 mt-1">Jika tidak dicentang, pengguna tidak akan bisa login.</p>
            </div>
