...
$table = $table ?? null;
// === GUNAKAN getFlashData ===
$oldInputFromSession = SessionHelper::getFlashData('old_input');
// ============================
if (!$table && !$oldInputFromSession) { /* ... handle error ... */ return; }
...
<div class="max-w-lg mx-auto">
    <div class="flex justify-between items-center mb-6">
         <h2 class="text-2xl font-semibold text-slate-800">Edit Pengguna: <?= SanitizeHelper::html($user['name']) ?></h2>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md border border-slate-200">
        <form action="<?= UrlHelper::baseUrl('/admin/users/update/' . $user['id']) ?>" method="POST" novalidate>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select id="role" name="role" required
                         <?= $isEditingSelf ? 'disabled' : '' // Disable jika edit diri sendiri ?>
                 ...
                 <?php if($isEditingSelf): ?>
                    ...
                    <input type="hidden" name="role" value="<?= SanitizeHelper::html($formData['role']) ?>"> <?php endif; ?>
            </div>
            ...
            <div class="flex justify-end space-x-3">
                <a href="<?= UrlHelper::baseUrl('/admin/users') ?>" class="bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium py-2 px-4 rounded-lg transition border border-slate-300">
                    Batal
                </a>
